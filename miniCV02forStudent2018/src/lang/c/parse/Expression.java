...
class ExpressionAdd extends CParseRule {
	...
	public void semanticCheck(CParseContext pcx) throws FatalErrorException {
		...
		if (left != null) {
			...
			pcx.fatalError(op.toExplainString() + "左辺がありません");
		}
		if (right != null) {
			...
			pcx.fatalError(op.toExplainString() + "右辺がありません");
		}
...
class ExpressionSub extends CParseRule {
	...
	public void semanticCheck(CParseContext pcx) throws FatalErrorException {
		// 引き算の型計算規則
		final int s[][] = {
			//				整数ーポインタ＝ポインタ？番地が負になりうるのでERR　ポインター整数＝ポインタ	ポインタ-ポインタ＝配列の個数を調べるときなどに使用する
